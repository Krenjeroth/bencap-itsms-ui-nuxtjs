...
const { loading, hasError } = storeToRefs(itServiceStore);
...
const handleSubmit = async () => {
  await itServiceStore.deleteItService(props.itService?.id);
...
<template>
  <BaseModal
    :on-close="onClose"
    :title="`Delete ${props.itService?.name} (${props.itService?.code})`"
