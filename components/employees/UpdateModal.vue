...
const formState = ref<IUpdateEmployeeForm>({
  uid: props.employee?.uid || undefined,
  firstname: props.employee?.firstname || undefined,
  middlename: props.employee?.middlename || undefined,
  lastname: props.employee?.lastname || undefined,
  suffix: props.employee?.suffix || undefined,
  position: props.employee?.position_id || undefined,
  department: props.employee?.department_id || undefined,
});

const originalState = ref<IUpdateEmployeeForm>({
  ...cloneDeep({
    uid: props.employee?.uid || undefined,
    firstname: props.employee?.firstname || undefined,
    middlename: props.employee?.middlename || undefined,
...
const fieldsToCompare: (keyof IUpdateEmployeeForm)[] = [
  "uid",
  "firstname",
  "middlename",
  "lastname",
...
const handleSubmit = async (
  event: IFormSubmitEvent<TUpdateEmployeeValidationSchema>
) => {
  if (!isChangedComputed.value) {
    onNoDataChange();
    return;
  }
  await employeeStore.updateEmployee(props.employee?.id, event.data);
