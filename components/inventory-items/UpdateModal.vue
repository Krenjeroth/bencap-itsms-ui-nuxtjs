...
const { loading, errorBag, hasError } = storeToRefs(inventoryItemStore);
...
const formState = ref<IUpdateInventoryItemForm>({
...
const originalState = ref<IUpdateInventoryItemForm>({
  ...cloneDeep({
    brand_model: props.item?.brand_model || undefined,
    measurement_unit: props.item?.measurement_unit.id || undefined,
    description: props.item?.description || undefined,
...
const fieldsToCompare: (keyof IUpdateInventoryItemForm)[] = [
  "brand_model",
  "measurement_unit",
  "description",
  "item_number",
...
const handleSubmit = async (
  event: IFormSubmitEvent<TUpdateInventoryItemValidationSchema>
) => {
  ...
  await inventoryItemStore.updateInventoryItem(props.item?.id, event.data);
...
<template>
  ...
  >
    <UForm
      :schema="UpdateInventoryItemValidationSchema"
      :state="formState"
