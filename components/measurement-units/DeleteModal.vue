...
const { loading, hasError } = storeToRefs(measurementUnitStore);
...
const handleSubmit = async () => {
  await measurementUnitStore.deleteMeasurementUnit(props.measurementUnit?.id);
...
<template>
  <BaseModal
    :on-close="onClose"
    :title="`Delete ${props.measurementUnit?.name}`"
